...
static void zmk_rgb_underglow_tick(struct k_work *work) {
    ...
    case UNDERGLOW_EFFECT_SWIRL:
        ...
        LOG_INF("Updating underglow: STRIP_NUM_PIXELS=%d, CONFIG_LEDS=%d",
                STRIP_NUM_PIXELS, CONFIG_ZMK_RGB_UNDERGLOW);
        BUILD_ASSERT(STRIP_NUM_PIXELS == CONFIG_ZMK_RGB_UNDERGLOW,
                    "Mismatch: DT chain-length vs CONFIG_ZMK_RGB_UNDERGLOW_LEDS");
    #else
        LOG_INF("Updating underglow: STRIP_NUM_PIXELS=%d", STRIP_NUM_PIXELS);
    #endif

    if (STRIP_NUM_PIXELS > 0) {
        LOG_DBG("Pixel[0] R=%u G=%u B=%u", pixels[0].r, pixels[0].g, pixels[0].b);
        LOG_HEXDUMP_DBG(pixels, STRIP_NUM_PIXELS * sizeof(pixels[0]), "Pixel buffer");
    }
...
static int zmk_rgb_underglow_init(void) {
    ...
    if (!device_is_ready(led_strip)) {
        LOG_ERR("LED strip device %s NOT ready", led_strip->name);
        return -ENODEV;
    }
    LOG_INF("RGB underglow init: device=%s, pixels=%d", led_strip->name, STRIP_NUM_PIXELS);
